...
@section('content')
    <div class="page-title mb-3">
        <div>
            <h5 class="fw-600">Update Role</h5>
        </div>
    </div>

    <div class="page-wrapper mt-3">

        <form id="saveButton" action="{{ route('roles.update', $role->id) }}" method="POST"
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    ...
                    <select id="guard_name" name="guard_name" class="custom-select2">
                        <option @selected($role->guard_name == 'admin') value="admin">Admin</option>
                        {{-- <option @selected($role->guard_name == 'web') value="web">Web</option> --}}
                    </select>
                    @error('guard_name')
                        <div class="invalid-feedback d-block"> {{ $message }}</div>
                    @enderror
                                        ...
                                        <div class="col-4">
                                            <div class="form-check">
                                                <input class="form-check-input   group-checkbox" type="checkbox"
                                                    value="{{ $groupLoopId }}"
                                                    {{ $permissions->every(fn($permission) => $role->permissions->contains($permission->id)) ? 'checked' : '' }}
                                        ...
                                        <div class="col-8">
                                            @foreach ($permissions as $permission)
                                                <div class="form-check mb-2">
                                                    <input
                                                        class=" form-check-input substituted group_id{{ $groupLoopId }}"
